  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="border-b border-border-subtle bg-surface-base">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-[11px] font-semibold text-accent uppercase tracking-[0.1em] mb-1">
                Panel de Control
              </p>
              ...
              <p className="text-sm text-zinc-400 mt-1.5 max-w-lg">
                Centro de administración y monitoreo de la operación de buses.
              </p>
            </div>
            <div className="hidden sm:flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-success animate-pulse" />
              <span className="text-[11px] text-zinc-400 font-mono">Sistema operativo</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto w-full px-6 lg:px-8 py-8">
        <div className="space-y-10">
          {MODULES.map((col) => (
            <section key={col.category}>
              <h2 className="text-[11px] font-semibold text-zinc-500 uppercase tracking-[0.08em] mb-4">
                {col.category}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {col.items.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="group relative rounded-xl border border-border-default bg-surface-card p-5 transition-all duration-200 hover:border-border-strong hover:bg-surface-raised"
                  >
                    <div className="flex items-start gap-4">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-accent/8 text-accent group-hover:bg-accent/12 transition-colors">
                        {item.icon}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-zinc-600 transition-transform group-hover:translate-x-0.5 group-hover:text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Bottom Status */}
        <div className="mt-10 rounded-xl border border-border-subtle bg-surface-card p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-1.5 w-1.5 rounded-full bg-success" />
            <span className="text-[12px] text-zinc-400">Todos los servicios activos</span>
          </div>
          <span className="text-[11px] text-zinc-600 font-mono">
            {MODULES.reduce((acc, m) => acc + m.items.length, 0)} módulos disponibles
          </span>
        </div>
      </div>
    </div>
