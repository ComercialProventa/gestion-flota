...
export default function BusDetalleClient({ id }: { id: string }) {
  ...
  if (isError || !bus) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="bg-surface rounded-md p-4 text-red max-w-md">
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 border-b border-border bg-background bg-opacity-80 backdrop-blur-sm">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <a
              href="/admin/buses"
              className="flex h-9 w-9 items-center justify-center rounded-md text-dim hover:text-foreground hover:bg-surface transition-colors"
            >
            ...
            <div>
              <h1 className="text-lg font-semibold text-foreground font-mono tracking-wider">{bus.patente}</h1>
              <p className="text-xs text-muted">{bus.marca} {bus.modelo}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-4xl px-4 py-6 space-y-6">
        ...
        <div className="bg-surface rounded-md p-6">
          <h2 className="text-sm font-medium text-foreground mb-6 flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex justify-center gap-6 mt-6 pt-4 border-t border-border">
            <span className="flex items-center gap-1.5 text-xs text-dim">
              <span className="h-2.5 w-2.5 rounded-full bg-green" /> &gt; 30 días
            </span>
            <span className="flex items-center gap-1.5 text-xs text-dim">
              <span className="h-2.5 w-2.5 rounded-full bg-accent" /> 11–30 días
            </span>
            <span className="flex items-center gap-1.5 text-xs text-dim">
              <span className="h-2.5 w-2.5 rounded-full bg-red" /> ≤ 10 días
            </span>
          </div>
        </div>
...
function InfoItem({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <p className="text-[10px] uppercase tracking-wider text-dim mb-0.5">{label}</p>
      <p className={`text-sm font-medium text-foreground ${mono ? "font-mono tracking-wider" : ""}`}>
        {value}
      </p>
    </div>
  );
